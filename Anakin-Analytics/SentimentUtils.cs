...
namespace AnakinAnalytics
{
    internal static class SentimentUtils
    {
        ...
        public static Dictionary<string, double> BoosterDict = new Dictionary<string, double>
        {
            {"absolutely", BIncr},
            {"amazingly", BIncr},
            {"awfully", BIncr},
        ...
        public static Dictionary<string, double> SpecialCaseIdioms = new Dictionary<string, double>
        {
            {"the shit", 3},
            { "the bomb", 3},
            { "bad ass", 1.5},
        ...
        public static void ApplyConfig(ConfigStore config)
        {
            if (config == null)
                return;

            if (config.Negations != null && config.Negations.Length > 0)
                Negate = config.Negations;

            if (config.BoosterDict != null && config.BoosterDict.Count > 0)
                BoosterDict = new Dictionary<string, double>(config.BoosterDict, StringComparer.OrdinalIgnoreCase);

            if (config.SpecialCaseIdioms != null && config.SpecialCaseIdioms.Count > 0)
                SpecialCaseIdioms = new Dictionary<string, double>(config.SpecialCaseIdioms, StringComparer.OrdinalIgnoreCase);
        }
